...
const OrderRestaurant = ({ order_data, setModalVisible, navigation }) => {
  ...
  const handleDelivered = async () => {
    try {
      const add_response = await setDoc(
        doc(db, "DeliveredOrders", order_data.id),
        order_data
      );

      await deleteDoc(doc(db, "Orders", order_data.id));
      setModalVisible(() => {
        return { visible: true, order_id: order_data.id };
      });
    } catch (err) {}
  };
      ...
      <TouchableOpacity onPress={viewOrderDetails} style={styles.card}>
        <View style={{ textAlign: "center", marginRight: 10, marginTop: 10 }}>
          ...
          <Text style={{ color: "white", textAlign: "center" }}>
            {order_data.restaurant_name}
          </Text>
        </View>

        <View
          style={{ textAlign: "center", alignSelf: "center", marginRight: 2 }}
        >
          <Text style={{ color: "white", textAlign: "left" }}>
            Ordered: {order_date} min ago
          </Text>
        </View>
