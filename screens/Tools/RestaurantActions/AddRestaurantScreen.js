...
const AddRestaurantScreen = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (
        !restaurant_data.restaurant_name.trim() ||
        !restaurant_data.restaurant_adress.trim() ||
        !email.trim()
      )
        throw new Error("Please enter data to all fields!");
      ...
      if (userType == "user") {
        ...
        const user_to_restaurant_owner = await updateDoc(
          doc(db, "Users", uid),
          { user: "restaurant_owner" }
        );
      }

      const image_upload_response = await uploadImage(
        Imageuri,
        restaurant_data.restaurant_name,
        restaurant_response.id
      );
      ...
      {!loading ? (
        <ScrollView contentContainerStyle={styles.form}>
          ...
          <TextInput
            ...
            onChangeText={(val) =>
              setrestaurant_data((curr) => {
                return { ...curr, restaurant_name: val };
              })
            }
          />
          <TextInput
            style={[styles["input-field"], { marginBottom: 10 }]}
            placeholder="enter Owners email adress..."
          ...
          <TextInput
            ...
            onChangeText={(val) =>
              setrestaurant_data((curr) => {
                return { ...curr, restaurant_adress: val };
              })
            }
          />
          <TextInput
            ...
            onChangeText={(val) =>
              setrestaurant_data((curr) => {
                return { ...curr, restaurant_description: val };
              })
            }
          />
