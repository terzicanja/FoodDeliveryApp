...
const RestaurantSingle = ({ route, navigation }) => {
  ...
  const { id, restaurant_adress, restaurant_menu, restaurant_name } =
    restaurant_data;
  ...
  if (!restaurant_menu == null || !restaurant_menu == "") {
    const objArray = [];
    Object.keys(restaurant_menu).forEach((key) =>
      objArray.push({
        id: key,
  ...
  return (
    <Fragment>
      <View style={styles.container}>
        ...
        {Orders_container.length > 0 ? (
          <View style={styles.cart}>
            <Text style={{ width: "100%", textAlign: "center" }}>
              {Orders_container.length}{" "}
              {Orders_container.length > 1 ? "items" : "item"} in cart. Do you
              want to procced?
            </Text>

            <TouchableOpacity style={styles.order} onPress={handleOrder}>
              <Text style={{ color: "white", padding: 2 }}>Order</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.cancel}
              onPress={() => setOrders_container([])}
            >
              <Text style={{ color: "white", padding: 2 }}>Cancel</Text>
            </TouchableOpacity>
          </View>
        ) : null}
      </View>
