...
const Restaurant = ({ restaurant_data, navigation, screen }) => {
  ...
  const getImageFromStorage = async () => {
    try {
      ...
      const imgRef = ref(
        storage,
        restaurant_data.id + "/" + restaurant_data.restaurant_name
      );
  ...
  return (
    <Fragment>
      <TouchableOpacity
        style={styles.card}
        onPress={() => navigation.navigate(screen, { restaurant_data })}
      >
        <View style={{ width: "30%" }}>
          <Image
            source={{
            ...
            style={{ width: 70, height: 70, left: 5, top: 5, borderRadius: 10 }}
          />
        </View>
